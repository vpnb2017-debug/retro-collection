...
import { localFileSync } from './services/localFileSync.js?v=80';
import { metadataService } from './services/metadataService.js?v=80';
import { cloudSyncService } from './services/cloudSyncService.js?v=80';
...
async function renderDashboard() {
    ...
    try {
        ...
        titleEl.innerHTML = `<h2>Resumo <span style="font-size:0.6rem; color:#ff9f0a; border:1px solid; padding:2px 4px; border-radius:4px; margin-left:8px;">v80</span></h2>`;
...
/** INITIALIZATION **/
async function init() {
    logger("Iniciando RetroCollection v80...");
    try {
        await dbService.open();
        logger("DB Conectado.");

        // Auto-Sync Logos logic for v80
        if (!localStorage.getItem('logos_synced_v80')) {
            await autoSyncLogos();
            localStorage.setItem('logos_synced_v80', 'true');
        }
